        ...
        match expr {
            ...
            Expr::Call { name, args } => {
                ...
                match (name.as_str(), args.as_slice()) {
                    ...
                    ("avg", [list_expr]) => {
                        let list_val = self.eval(list_expr, scope)?;
                        let list = match list_val { Value::List(v) => v, _ => return Err(EvalError::new("avg expects a list").into()) };
                        let mut total: f64 = 0.0;
                        let mut count: usize = 0;
                        for v in list.iter() {
                            match v {
                                Value::Int(n) => { total += *n as f64; count += 1; }
                                Value::Float(n) => { total += *n; count += 1; }
                                _ => return Err(EvalError::new("avg expects numeric list").into()),
                            }
                        }
                        return Ok(Value::Float(if count == 0 { 0.0 } else { total / count as f64 }));
                    }
                    ("avg_by", [list_expr, Expr::Lambda { param, body }]) => {
                        let list_val = self.eval(list_expr, scope)?;
                        let list = match list_val { Value::List(v) => v, _ => return Err(EvalError::new("avg_by expects list as first argument").into()) };
                        let mut total: f64 = 0.0;
                        let mut count: usize = 0;
                        for item in list.into_iter() {
                            let mut inner = scope.clone();
                            ...
                            match v {
                                Value::Int(n) => { total += n as f64; count += 1; }
                                Value::Float(n) => { total += n; count += 1; }
                                _ => return Err(EvalError::new("avg_by lambda must return number").into()),
                            }
                        }
                        return Ok(Value::Float(if count == 0 { 0.0 } else { total / count as f64 }));
                    }
                    ("min_of", [list_expr]) => {
                        let list_val = self.eval(list_expr, scope)?;
                        let list = match list_val { Value::List(v) => v, _ => return Err(EvalError::new("min_of expects a list").into()) };
                        let mut best: Option<f64> = None;
                        for v in list.iter() {
                            let x = match v {
                                Value::Int(n) => *n as f64,
                                Value::Float(n) => *n,
                                _ => return Err(EvalError::new("min_of expects numeric list").into()),
                            };
                            best = Some(match best { Some(b) => b.min(x), None => x });
                        }
                        return Ok(Value::Float(best.unwrap_or(0.0)));
                    }
                    ("min_by", [list_expr, Expr::Lambda { param, body }]) => {
                        let list_val = self.eval(list_expr, scope)?;
                        let list = match list_val { Value::List(v) => v, _ => return Err(EvalError::new("min_by expects list as first argument").into()) };
                        let mut best: Option<f64> = None;
                        for item in list.into_iter() {
                            let mut inner = scope.clone();
                            inner.vars.insert(param.clone(), item);
                            ...
                            let x = match v {
                                Value::Int(n) => n as f64,
                                Value::Float(n) => n,
                                _ => return Err(EvalError::new("min_by lambda must return number").into()),
                            };
                            best = Some(match best { Some(b) => b.min(x), None => x });
                        }
                        return Ok(Value::Float(best.unwrap_or(0.0)));
                    ...
                    ("max_of", [list_expr]) => {
                        let list_val = self.eval(list_expr, scope)?;
                        let list = match list_val { Value::List(v) => v, _ => return Err(EvalError::new("max_of expects a list").into()) };
                        let mut best: Option<f64> = None;
                        for v in list.iter() {
                            let x = match v {
                                Value::Int(n) => *n as f64,
                                Value::Float(n) => *n,
                                _ => return Err(EvalError::new("max_of expects numeric list").into()),
                            };
                            best = Some(match best { Some(b) => b.max(x), None => x });
                        }
                        return Ok(Value::Float(best.unwrap_or(0.0)));
                    }
                    ("max_by", [list_expr, Expr::Lambda { param, body }]) => {
                        let list_val = self.eval(list_expr, scope)?;
                        let list = match list_val { Value::List(v) => v, _ => return Err(EvalError::new("max_by expects list as first argument").into()) };
                        let mut best: Option<f64> = None;
                        for item in list.into_iter() {
                            let mut inner = scope.clone();
                            inner.vars.insert(param.clone(), item);
                            ...
                            let x = match v {
                                Value::Int(n) => n as f64,
                                Value::Float(n) => n,
                                _ => return Err(EvalError::new("max_by lambda must return number").into()),
                            };
                            best = Some(match best { Some(b) => b.max(x), None => x });
                        }
                        return Ok(Value::Float(best.unwrap_or(0.0)));
